...
func getArgumentsFromMemory(
	...
	metadataGas := math.MulUint64(metering.GasSchedule().BaseOperationCost.DataCopyPerByte, uint64(numArguments)*4)
	if metering.GasLeft() < metadataGas {
		return nil, 0, vmhost.ErrNotEnoughGas
	}
	metering.UseGas(metadataGas)

	argumentsLengthData, err := runtime.MemLoad(argumentsLengthOffset, numArguments*4)
	if err != nil {
		metering.RestoreGas(metadataGas)
		return nil, 0, err
	}

	argumentLengths := createInt32Array(argumentsLengthData, numArguments)

	totalArgumentBytes := int64(0)
	for _, length := range argumentLengths {
		if length < 0 {
			metering.RestoreGas(metadataGas)
			return nil, 0, fmt.Errorf("negative argument length (%d)", length)
		}
		totalArgumentBytes += int64(length)
	}

	argsGas := math.MulUint64(metering.GasSchedule().BaseOperationCost.DataCopyPerByte, uint64(totalArgumentBytes))
	if metering.GasLeft() < argsGas {
		metering.RestoreGas(metadataGas)
		return nil, 0, vmhost.ErrNotEnoughGas
	}
	metering.UseGas(argsGas)

	data, err := runtime.MemLoadMultiple(dataOffset, argumentLengths)
	if err != nil {
		metering.RestoreGas(metadataGas + argsGas)
		return nil, 0, err
	}

	metering.RestoreGas(metadataGas + argsGas)

	return data, int32(totalArgumentBytes), nil
}
